...
select results_eq(
  ...
  $$values (11::bigint)$$,
  'authenticated INSERT grants remain for invoker command boundaries'
);
select results_eq(
  ...
  $$values (9::bigint)$$,
  'authenticated UPDATE excludes task_events and immutable feedback'
);
select results_eq(
  ...
  $$values (7::bigint)$$,
  'authenticated DELETE excludes event, plan history, plan items, and feedback'
);
select results_eq(
...
select results_eq(
  ...
  $$values (2::bigint)$$,
  'task_events retains owner policies while trigger guard enforces command context'
);
...
values (current_setting('wp003.event_a')::uuid,current_setting('wp003.user_a')::uuid,current_setting('wp003.task_keep_a')::uuid,'started',now(),'user');
...
select lives_ok(
  ...
  'User A can update ordinary Task definition fields'
);
select lives_ok(
  $$delete from public.tasks where id=current_setting('wp003.task_crud_a')::uuid$$,
  'User A can delete own Task definition under existing management boundary'
);
select results_eq(
...
select throws_ok(
  $$insert into public.task_events (id,user_id,task_id,event_type,occurred_at,actor) values (gen_random_uuid(),current_setting('wp003.user_a')::uuid,current_setting('wp003.task_keep_a')::uuid,'started',now(),'user')$$,
  'P0001','Execution mutations must use record_task_action_v01.','direct own Task Event INSERT is denied by command guard'
);
select results_eq(
  $$select count(*)::bigint from public.task_events where id=current_setting('wp003.event_a')::uuid$$,
  $$values (1::bigint)$$,
  'User A can select own existing Task Event'
);
select throws_ok(
...
select throws_ok(
  ...
  'P0001','Execution mutations must use record_task_action_v01.','direct cross-owner Task Event INSERT cannot bypass command boundary'
);
